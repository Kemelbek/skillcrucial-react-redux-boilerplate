...
echo.on('connection', (conn) => {
  ...
  fs.readFile(fileName,
    (err) => {
      if (err) {
        const GenData = new Array(100).fill(null).map(getFakerUser)
        return fs.writeFile(fileName, JSON.stringify(GenData))
      }
      return null
    })
});
  ...
  fs.readFile(fileName,
    (err, data) => {
      if (!err) {
        return res.json(
          {
            TotalPages: (JSON.parse(data).length / PAGE_SIZE),
            list: JSON.parse(data).slice(+pageIndex * PAGE_SIZE, (+pageIndex + 1) * PAGE_SIZE)
          }
        )
      }
      const GenData = new Array(100).fill(null).map(getFakerUser)
